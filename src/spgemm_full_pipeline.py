...
def load_ss_matrix(selector: str, dest_dir: str) -> tuple[csr_matrix, ssgetpy.Matrix]:
    ...
    if len(rs) == 0:
        raise ValueError(f"[ssgetpy] Matrix not found: {selector}")
...
def mesh_dims(num_nodes: int) -> Tuple[int, int]:
    """
    Find (p, q) such that p*q == num_nodes and |p-q| is minimized.
    Falls back to 1 x num_nodes if num_nodes is prime.
    """
    if num_nodes <= 0:
        return 1, 1
...
def generate_2d_mesh_metis(num_nodes: int, out_path: str) -> None:
    """
    Generate a 4-neighbor 2D mesh METIS graph for MtKaHyPar -g.
    Nodes are row-major, indexed 0..(p*q-1), with total count num_nodes.
    """
...
# ================== 3. Hypergraph Construction (weighted by B-row nnz) ==================
...
def build_gustavson_hypergraph_for_A(
    ...
    """
    Vertices: every row of A (a Gustavson row task)
    Hyperedges: each column k of A, containing rows where A[i, k] != 0

    If use_b_nnz_as_edge_weight=True, use nnz of row k in B as the hyperedge
    weight in hMETIS / Mt-KaHyPar format:
      line 1: <num_hyperedges> <num_vertices> 1
      subsequent lines: <w_e> v1 v2 ...
    """
    A_csc = A.tocsc()
    B_csr = B.tocsr()
    ...
    for k in range(n_cols):
        ...
        if rows.size == 0:
            continue
        he = (rows + 1).tolist()  # vertices are 1-based
...
# ================== 4. WaferSpMM baseline + Mt-KaHyPar partitioning ==================

def build_wafer_partition(n_rows: int, num_parts: int) -> np.ndarray:
    """
    WaferSpMM baseline: simple row-block mapping.
    """
...
def build_owner_B_wafer(n_rows_b: int, num_parts: int, mode: str) -> np.ndarray:
    """
    B-row ownership for the WaferSpMM baseline:
      - rowblock: contiguous row blocks
      - mod: k % num_parts
    """
    if mode == "rowblock":
...
def build_owner_B_hypergraph_center(
    ...
    """
    HyperWafer B-row ownership:
      For each column k (matching row k in B):
        - collect rows with A[i, k] != 0;
        - inspect which tiles those rows map to via row_part;
        - pick the tile with the highest frequency as owner_B[k].
    """
    A_csc = A.tocsc()
    n_rows_b = A.shape[1]
...
# ================== 5. Exact Bcast simulation on a 2D mesh (oracle) ==================
...
def shortest_path_mesh(src: int, dst: int, p: int, q: int):
    ...
    edges = []
    cr, cc = r1, c1
    # Manhattan path, rows first then columns
    if r2 != cr:
...
def simulate_bcast_on_mesh(
    ...
    """
    Precisely measure per-link traffic and GB-hop for B-row multicasts on a 2D mesh.
    """
...
def collect_bcast_row_bytes(
    ...
    """
    Compute one broadcast volume for each row B[k, :]:
...
def build_bcast_micro_phases_rows_batch(
    ...
    """
    Batch rows by count:
      - accumulate row_bytes sequentially until rows_per_layer entries form a layer;
      - keep the last partial layer if needed;
      - no zero padding or forced layer count, matching the batch-512 intuition.
    """
...
def run_astrasim_and_get_comm_time(
    ...
    """
    Invoke the AstraSim analytical backend and parse the max communication time.
    """
...
def main():
    ...
    default_ss_dir = str(Path.home() / ".cache" / "ssgetpy")

    # Matrix and basic parameters
    parser.add_argument(
        "--matrix-selector",
        required=True,
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "               HyperWafer uses the same rows_per_layer (therefore it usually has fewer layers); "
    ...
    # 3. HyperWafer mapping (hypergraph + Mt-KaHyPar with 2D mesh target and B-row weights)
    ...
    # Build B-row owners based on hypergraph partitions (use the tile with the highest task count)
    ...
    # 7. AstraSim: WaferSpMM / HyperWafer communication time
